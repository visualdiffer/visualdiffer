...
public extension BaseTests {
    func appendFolder(_ path: String, _ isFolder: Bool = true, functionName: String = #function) -> URL {
        rootDir
            .appending(path: pathComponent(functionName: functionName), directoryHint: .isDirectory)
    ...
    func pathComponent(functionName: String) -> String {
        functionName.trimmingCharacters(in: CharacterSet(charactersIn: "()"))
    }
}
